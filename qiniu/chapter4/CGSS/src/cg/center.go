...
// Ensure the CenterServer implements all the method defined in ipc.Server
...
func (server *CenterServer) addPlayer(params string) error {
	...
	// TODO check duplicated user before appending
	...
	for i, v := range server.players {
		if v.Name == params {
			if len(server.players) == 1 {
				...
				server.players = append(server.players[:i], server.players[i+i:]...)
			}
			return nil
		}
	}
...
/* implements interface ipc.Server */
